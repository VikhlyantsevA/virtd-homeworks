...
<br>
Результат curl запроса `(Так и не удалось этот момент победить. Как я понял докер хост запущен на виртуальной машине и достучаться до него с моего mac на котором запущен Docker Desktop невозможно, но мб тут вы мне предложите решение. В моем случае приходилось запускать оболочку bash в контейнере с ingress и из него делать curl на 127.0.0.1:8090. Однако и ip был соответствующий 127.0.0.1. Возможно предполагалось что задание будет запускаться на виртуалке с Linux и там все нормально, но у меня такой возможности нет, ибо mac на M1 и тот же самый virtualBox не работает, а Qemu очень перегружает комп.):`
...
Пропустил, т.к. не понялл что требуется в этом задании.
